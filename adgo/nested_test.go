...
func TestRunNestedLocalResumesSameParentApplication(t *testing.T) {
	...
	definition := Definition{ID: "nested.test", Version: "1", Nodes: []Node{{ID: "work", Kind: NodeActivity, Activity: "work", Produces: []string{"done"}}}}
	...
	if err != nil { t.Fatal(err) }
	if first.ID != second.ID { t.Fatalf("child identity changed: %q != %q", first.ID, second.ID) }
	if calls.Load() != 1 { t.Fatalf("activity calls=%d, want 1", calls.Load()) }
}
...
func TestRunNestedLocalChangesIdentityWithParentApplication(t *testing.T) {
	...
	definition := Definition{ID: "nested.test.revision", Version: "1", Nodes: []Node{{ID: "work", Kind: NodeActivity, Activity: "work", Produces: []string{"done"}}}}
	base := NestedInvocation{ParentExecutionID: "parent", ParentNodeID: "drafting", FlowName: "writer"}
	firstInvocation := base; firstInvocation.ParentApplicationID = "parent:drafting:r1:input-a"
	secondInvocation := base; secondInvocation.ParentApplicationID = "parent:drafting:r2:input-a"
	...
	if err != nil { t.Fatal(err) }
	if first.ID == second.ID { t.Fatalf("new parent application reused child %q", first.ID) }
	if calls.Load() != 2 { t.Fatalf("activity calls=%d, want 2", calls.Load()) }
}

func TestNestedExecutionIDDoesNotExposeApplicationMaterial(t *testing.T) {
	id, err := NestedExecutionID(NestedInvocation{ParentExecutionID: "parent", ParentNodeID: "node", FlowName: "writer", ParentApplicationID: "secret-looking-logical-input-that-must-not-be-in-id"})
	if err != nil { t.Fatal(err) }
	if id == "" { t.Fatal("empty child id") }
	if containsString(id, "secret-looking") { t.Fatalf("nested id leaks application material: %q", id) }
}
...
func TestChildExecutionIDIsCanonicalAcrossHostAndNestedHelpers(t *testing.T) {
	...
	if got != want { t.Fatalf("ChildExecutionID=%q, want %q", got, want) }
}
...
func TestNestedExecutionIDUsesCanonicalChildPrefix(t *testing.T) {
	...
	if len(id) <= len(prefix) || id[:len(prefix)] != prefix { t.Fatalf("nested id=%q, want prefix %q", id, prefix) }
}
