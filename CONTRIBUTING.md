...
6. **Правило красного `main` (Red-Main Invariant):**
   - Красный базовый `main` блокирует любое несвязанное добавление фичей, рефакторинг и оптимизацию.
   - Падение любого матричного джоба (Linux, Windows, macOS) в CI считается production-регрессией, даже если Linux и race detector зелёные.
   - Следующий коммит после падения обязан либо устранять первопричину, либо откатывать регрессионное изменение.
   - Запрещено отключать тесты (`t.Skip`, удаление тестов, ослабление ассертов) только ради восстановления зелёного статуса.
